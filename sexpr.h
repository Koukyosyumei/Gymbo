...
inline std::shared_ptr<Expr> literalElimination(std::shared_ptr<Expr> expr) {
...
inline std::shared_ptr<Expr> unitPropagation(std::shared_ptr<Expr> expr) {
...
inline bool satisfiable(std::shared_ptr<Expr> expr) {
  std::pair<bool, std::string> freevar = expr->freeVar();
  if (!freevar.first) {
    return expr->unConst();
  } else {
    ...
    return satisfiable(trueGuess) || satisfiable(falseGuess);
  }
}

inline bool satisfiableDPLL(std::shared_ptr<Expr> expr) {
  std::shared_ptr<Expr> expr2 = literalElimination(cnf(unitPropagation(expr)));
  ...
  if (!freevar.first) {
    ...
    auto trueGuess = expr->guessVar(var, true)->simplify();
    auto falseGuess = expr->guessVar(var, false)->simplify();
    return satisfiableDPLL(trueGuess) || satisfiableDPLL(falseGuess);
  }
}
